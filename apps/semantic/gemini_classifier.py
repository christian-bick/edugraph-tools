...
class GeminiClassifier:
    ...
    def classify_area(self, description, classifier):
        descriptor_type = self.onto.Area
        nodes = [ self.onto.Mathematics ]
        context = self.context_builder.build_area_context(nodes)
        area = classifier.find_best_match(description, context, descriptor_type)
        return area

    def classify_ability(self, description, classifier):
        ...
        matched_areas = classifier.find_matches(description, context, descriptor_type)
        return matched_areas

    def classify_scope(self, description, classifier):
        ...
        matched_scopes = classifier.find_matches(description, context, descriptor_type)
    ...
    def classify_content(self, file):
        ...
        description = classifier.describe_content()
        return self.classify_text(description, classifier)

    def classify_text(self, text, classifier):
        classification = {
            "area": self.classify_area(text, classifier),
            "abilities": self.classify_ability(text, classifier),
            "scopes": self.classify_scope(text, classifier)
        }
